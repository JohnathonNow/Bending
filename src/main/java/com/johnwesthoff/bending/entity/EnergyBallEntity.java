...
public class EnergyBallEntity extends Entity {
    ...
    public EnergyBallEntity(int x, int y, int hspeed, int vspeed, int ma) {
        X = x;
        Y = y;
        xspeed = hspeed;
        yspeed = vspeed;
    ...
    public void onServerUpdate(Server lol) {
        if (lol.earth.checkCollision(X, Y)) {
            radius = 12;
            lol.earth.ground.fillCircleW((int) X, (int) Y, radius, Constants.ENERGY);
            lol.sendMessage(FillEvent.getPacket((int) (X), (int) (Y), radius, Constants.ENERGY));
            alive = false;
        }
    }
    ...
    public static void reconstruct(ByteBuffer in, World world) {
        try {
            world.entityList.add(new EnergyBallEntity(in.getInt(), in.getInt(), in.getInt(), in.getInt(), in.getInt()));
        } catch (Exception ex) {
            Logger.getLogger(EnergyBallEntity.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
