...
public class PumpkinEntity extends Entity {
    ...
    public void onDraw(Graphics G, int viewX, int viewY) {
        if (X > viewX && X < viewX + Constants.WIDTH_INT && Y > viewY && Y < viewY + Constants.HEIGHT_INT) {
            ...
            if (near) {
                G.setColor(Color.BLACK);
                G.drawString("PRESS C TO UNLOCK",
                        (int) X - (G.getFontMetrics().stringWidth("PRESS C TO UNLOCK") / 2) - viewX,
                        (int) Y - 32 - viewY);
            }
        }
    }
        ...
        if (Math.abs(apples.x - X) < 64) {
            if (Math.abs(apples.y - Y) < 64) {
                ...
                if (apples.keys[KeyEvent.VK_C]) {
                    alive = false;
                    // Session.unlocks.set(0, 1, true);
                    // Session.CTD.postRSSfeed(Session.username + " found a rare item!","A Viking
                    // Helmet!");
                }
            }
        }
