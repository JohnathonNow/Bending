...
import com.johnwesthoff.bending.entity.PoisonBallEntity;
import com.johnwesthoff.bending.logic.World;
import com.johnwesthoff.bending.spells.Spell;
import com.johnwesthoff.bending.util.math.Ops;

public class ChemistryPoison extends Chemistry {
    public ChemistryPoison() {
        ID = Constants.CHEMISTRY;
        subID = 0;
        try {
            icon = (loadIcon("poison.png"));
        } catch (Exception ex) {
    ...
    public void getActionNetwork(World world, int px, int py, int mx, int my, int pid, int eid, ByteBuffer buf) {
        world.entityList.add(new PoisonBallEntity(px, py, mx, my, pid).setID(eid));
    }
}
