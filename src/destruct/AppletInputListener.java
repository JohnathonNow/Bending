...
import static destruct.APPLET.shortJump;
...
//                }
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        double scale = pointer.owner.getHeight()/300d;
        pointer.world.mouseX = (int)((scale * pointer.getWidth()) * ((e.getPoint().x-(pointer.getWidth()-pointer.getHeight()))/(double)pointer.getWidth()));
        //world.mouseX = ((e.getX()-(this.getWidth()-getHeight())/2)/scale);
        pointer.world.mouseY = (int)((scale * pointer.getHeight()) * (e.getPoint().y/(double)pointer.getHeight()));
        pointer.world.mouseX = (int)((e.getX()+getStuff())/scale);
        pointer.world.mouseY = (int)(e.getY()/scale);
            ...
            switch (e.getButton())
            {
                    ...
                    if (pointer.world.mouseX<172&&pointer.world.mouseY<17)
                    {
                        ...
                        if (setTo==-1)
                        {
                            if (pointer.energico>=pointer.spellList[pointer.spellBook][pointer.leftClick].getCost())
                            {
                                ...
                                pointer.world.leftArmAngle=APPLET.pointDir(pointer.world.left==1?(pointer.world.x-pointer.world.viewX):pointer.world.mouseX,pointer.world.y-pointer.world.viewY,pointer.world.left==-1?(pointer.world.x-pointer.world.viewX):pointer.world.mouseX,pointer.world.mouseY);
                                if ((pointer.passiveList[pointer.spellBook].getName().equals("Fire Charge"))&&(pointer.spellList[pointer.spellBook][pointer.leftClick] instanceof Spell.Firebending))
                                {
                    ...
                    if (pointer.world.mouseX<172&&pointer.world.mouseY<17)
                    {
                        ...
                        if (setTo==-1)
                        {
                            if (pointer.energico>=pointer.spellList[pointer.spellBook][pointer.midClick].getCost())
                            {
                                ...
                                pointer.world.leftArmAngle=APPLET.pointDir(pointer.world.left==1?(pointer.world.x-pointer.world.viewX):pointer.world.mouseX,pointer.world.y-pointer.world.viewY,pointer.world.left==-1?(pointer.world.x-pointer.world.viewX):pointer.world.mouseX,pointer.world.mouseY);
                                pointer.world.rightArmAngle=APPLET.pointDir(pointer.world.left==1?(pointer.world.x-pointer.world.viewX):pointer.world.mouseX,pointer.world.y-pointer.world.viewY,pointer.world.left==-1?(pointer.world.x-pointer.world.viewX):pointer.world.mouseX,pointer.world.mouseY);
                                if ((pointer.passiveList[pointer.spellBook].getName().equals("Fire Charge"))&&(pointer.spellList[pointer.spellBook][pointer.leftClick] instanceof Spell.Firebending))
                                {
                                    if (pointer.random.nextInt(5-doublecast)==0)
                                    {
                ...
                case MouseEvent.BUTTON3:
                    ...
                    if (pointer.world.mouseX<172&&pointer.world.mouseY<17)
                    {
                        ...
                        if (setTo==-1)
                        {
                            if (pointer.energico>=pointer.spellList[pointer.spellBook][pointer.rightClick].getCost())
                            {
                                pointer.energico-=pointer.spellList[pointer.spellBook][pointer.rightClick].getCost();
                                pointer.world.rightArmAngle=APPLET.pointDir(pointer.world.left==1?(pointer.world.x-pointer.world.viewX):pointer.world.mouseX,pointer.world.y-pointer.world.viewY,pointer.world.left==-1?(pointer.world.x-pointer.world.viewX):pointer.world.mouseX,pointer.world.mouseY);
                                if ((pointer.passiveList[pointer.spellBook].getName().equals("Fire Charge"))&&(pointer.spellList[pointer.spellBook][pointer.rightClick] instanceof Spell.Firebending))
                                {
                                    if (pointer.random.nextInt(5-doublecast)==0)
                                    {
        ...
        catch (Exception ex)
        {
          ...
          // if (pointer.world!=null){pointer.world.mouseX = (e.getX()*scale)+pointer.world.viewX+(pointer.getWidth()-pointer.getHeight())/2; pointer.world.mouseY = (e.getY()*scale)+pointer.world.viewY;}
    }
