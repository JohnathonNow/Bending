...
//        G2D.setPaint(null);
   ...
   /*                 entityList.add(new HouseEntity(450,150,20,20));
        entityList.add(new HouseEntity(500,140,20,20));
        entityList.add(new HouseEntity(400,140,20,20).setLook(Color.red, Color.ORANGE, Color.gray, Color.cyan, true));
        entityList.add(new HouseEntity(750,150,30,60));
     */  

        
        //entityList.add(new HouseEntity(750,899,60,30).setLook(Color.red, Color.darkGray, Color.blue, Color.cyan, true));
        }
        else
        {
        ...
        Runnable getStuff = new Runnable()
                {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            {
                                bodyParts[i] = ResourceLoader.loadImage("https://west-it.webs.com/bodyParts/p"+(i+1)+"_"+parts[i]+".png", "p"+(i+1)+"_"+parts[i]+".png");
...
//                                    }
                            ...
                            for (int e =-1; e<=1; e++)
                                {
                                    if (inBounds(x+e,y+1)&&cellData[x+e][y+1]==OIL&&cellData[x][y]==WATER)
                                    {
       ...
       public void freeze(int X, int Y, int R)
        ...
        //    long time = System.nanoTime();
            for (int i1 = Math.max(X-(R+1),0); i1 < Math.min(X+(R+1),w); i1 ++)
            {
                for (int i2 = Math.max(Y-(R+1),0); i2 < Math.min(Y+(R+1),h); i2 ++)
                {
                    if (Math.round(Math.sqrt(Math.pow(i1-X,2)+Math.pow(i2-Y,2))) < (R/2)+.1&&cellData[i1][i2]==WATER)
                    {
                        cellData[i1][i2]=ICE;
                    }
                }
        ...
        //    long time = System.nanoTime();
            for (int i1 = Math.max(X-(R+1),0); i1 < Math.min(X+(R+1),w); i1 ++)
            {
                for (int i2 = Math.max(Y-(R+1),0); i2 < Math.min(Y+(R+1),h); i2 ++)
                {
                    if (Math.round(Math.sqrt(Math.pow(i1-X,2)+Math.pow(i2-Y,2))) < (R/2)+.1&&cellData[i1][i2]==AIR)
                    {
       ...
       public void FillRectW(int X, int Y, int H, int W, byte with)
        {
           ...
           public void generater(int x, int w, byte type)
           {
             /*  for (int i = 0; i <= 30; i++)
               ...
               for (int e = x; e < x+w; e++)
               {
                   ...
                   for (int i = maGenH; i < h; i++)cellData[e][i] = type;
                   maGenH+=vs;
                   vs+=grav;
                   if (e % time == 0)
                   {
           ...
           public void generatel(int x, int w, byte type)
           {
               ...
               for (int e = x; e < x+w; e++)
               {
                   ...
                   for (int i = miGenH; i < h; i++)cellData[e][i] = type;
                   miGenH+=vs;
                   vs+=grav;
                   if (e % time == 0)
                   {
        ...
        //    long time = System.nanoTime();
            for (int i1 = Math.max(X,0); i1 < Math.min(X+W,w); i1 ++)
            {
                for (int i2 = Math.max(Y,0); i2 < Math.min(Y+H,h); i2 ++)
                {
                    if (cellData[i1][i2]==GROUND) cellData[i1][i2]=AIR;
                }
            }
//            G2D.setColor(Color.white);
  //      G2D.setPaint(skyPaint);
        ...
        /*if (MB1)
            {
                ...
                entityList.add(new MissileEntity(x,y,(int)lengthdir_x(dis,dir),(int)lengthdir_y(dis,dir)));
            }
            if (MB3)
            {
                MB3 = false;
...
try            {ground.handleWater();} catch (Exception e){e.printStackTrace();};
try            {drawTerrain((Graphics2D)g);} catch (Exception e){e.printStackTrace();};
       ...
       //     System.out.println(ddd);
            if (ddd==48)
            {
                ...
                g2.setTransform(previousAT);
                 ...
                 g2.translate((((x-3-viewX)+this.lengthdir_x(6*left, leftArmAngle*left)))*left*2.95+((left-1)*10), (((y-yUp-6)-this.lengthdir_y(6*left,leftArmAngle*left))-viewY)*3);
                ...
                g2.translate((((x+9-viewX)+this.lengthdir_x(6*left, rightArmAngle*left))*2.95)*left+((left-1)*10), (((y-yUp-6)-this.lengthdir_y(6*left,rightArmAngle*left))-viewY)*3);
            ...
            else
            {
               g2.translate((x-3-viewX)*3*left, ((y-yUp-6)-viewY)*3);
                ...
                g2.setTransform(previousAT);
                  ...
                  g2.translate((((x-3-viewX)+this.lengthdir_x(6*left, leftArmAngle*left)))*left*3, (((y-yUp-6)-this.lengthdir_y(6*left,leftArmAngle*left))-viewY)*3);
                    g2.rotate(Math.toRadians(leftArmAngle-90),4*(left+1),2);
                        g2.drawImage(bodyParts[3], 0,0, null);
                g2.setTransform(previousAT);
            }
    ...
    movePlayers();
        for (Player r:playerList)
        {
            ...
            if (((r.status&ST_DRAIN))!=0)
            {
                ...
                g.drawArc((r.x-viewX-(AURA_RADIUS/2))*3, (r.y-viewY-(AURA_RADIUS))*3, AURA_RADIUS*3, AURA_RADIUS*3, random.nextInt(360), random.nextInt(90));
            }
        }
}
            }
...
//        }
        ...
        for (int X = xx; X<xx+300; X++)
        {
            for (int Y = yy; Y<yy+300; Y++)
            {
                switch (ground.cellData[X][Y])
                {
                    default:
                    break; case GROUND:
                    Iter.setRGB(Math.min(X+3-xx,300), Math.min(Y+3-yy,300), Grass.getRGB(X % landTexSize, Y % landTexSize));
                    break; case SAND:
                    Iter.setRGB(Math.min(X+3-xx,300), Math.min(Y+3-yy,300), Sand.getRGB(X % landTexSize, Y % landTexSize));
                    break; case STONE:
                    Iter.setRGB(Math.min(X+3-xx,300), Math.min(Y+3-yy,300), Stone.getRGB(X % landTexSize, Y % landTexSize));
                    break; case TREE:
                    Iter.setRGB(Math.min(X+3-xx,300), Math.min(Y+3-yy,300), Bark.getRGB(X % landTexSize, Y % landTexSize));
                    break; case ICE:
    ...
    public void generateSandAndWater()
    {
        for (int i = 0; i<=6; i++)
        {
            entityList.add(new WaterEntity(random.nextInt(wIdTh),random.nextInt(hEigHt),this,random.nextBoolean()?SAND:WATER));
        }
    }
...
//image.getColorModel().
        ...
        g.dispose();
        int[] colors = image.getRGB(0, 0, image.getWidth(), image.getHeight(), null, 0, 1);
        ...
        return destImage;
            int[] colors = image.getRGB(0, 0, image.getWidth(), image.getHeight(), null, 0, 1);
            int colorChange = mask.getRGB();
            int colorTo = replacement.getRGB();
            for (int i = 0; i < colors.length; i++)
            {
                if (colors[i] == colorChange)
                {
                    colors[i] = colorTo;
                }
            }
            image.setRGB(0, 0, image.getWidth(), image.getHeight(), colors, 0, 1);
            return image;*/
