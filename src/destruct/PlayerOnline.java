...
public class PlayerOnline extends Player implements Runnable{
    Socket playerSocket;
    InputStream in; OrderedOutputStream out;
        ...
        switch (read)
        {
            case Server.LOGIN:
               ...
               // handle.newPlayer(ID, username);
                ...
                for (PlayerOnline p:handle.playerList)
                {
                    if (p.ID!=ID)
                    {
                        byte sameTeam = (byte)((handle.team1.contains(ID)&&handle.team1.contains(p.ID))||(handle.team2.contains(ID)&&handle.team2.contains(p.ID))?12:0);
                        out.addMesssage(Server.putString(ByteBuffer.allocate(75+4+4+4*p.username.length()).putInt(p.ID),p.username).put(p.cloth).putInt(p.colord[0]).putInt(p.colord[1]).putInt(p.colord[2]).putInt(p.colord[3]).putInt(p.colord[4]).putInt(p.colord[5]).putInt(p.colord2[0]).putInt(p.colord2[1]).putInt(p.colord2[2]).putInt(p.colord2[3]).putInt(p.colord2[4]).putInt(p.colord2[5]).put(sameTeam), Server.LOGIN);
                        p.out.addMesssage(Server.putString(ByteBuffer.allocate(75+4+4+4*username.length()).putInt(ID),username).put(clothing).putInt(colors[0]).putInt(colors[1]).putInt(colors[2]).putInt(colors[3]).putInt(colors[4]).putInt(colors[5]).putInt(colors2[0]).putInt(colors2[1]).putInt(colors2[2]).putInt(colors2[3]).putInt(colors2[4]).putInt(colors2[5]).put(sameTeam), Server.LOGIN);
                    }
                }
                loggedIn = true;
            break;
            ...
            case Server.MESSAGE:
                //System.out.println("MESSAGE RECIEVED!");
                 ...
                 if (yes.contains("/nextmap"))
                 {
                     ...
                     {
                        handle.nextVote++;
                        yes = (handle.nextVote+((handle.nextVote>1?" players are ":" player is ")+"voting for ending the match."));
                        voted = true;
                     }
                 }
                 handle.sendMessage(Server.MESSAGE, Server.putString(ByteBuffer.allocate(yes.length()*4+4).putInt(color),yes));
           // System.out.println(new String(buff).trim());
            ...
            case Server.AIRBENDING:
                 ...
                 handle.sendMessage(Server.AIRBENDING,ByteBuffer.allocate(28).putInt(subID).putInt(Xx).putInt(Yy).putInt(mX).putInt(mY).putInt(ID).putInt(Iw));
            break;
            case Server.EARTHBENDING:  
                 ...
                 switch (subID)
                 {
                     ...
                     case 4:
                         ...
                         if (number > 3)
                             {
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,30),mY+(int)APPLET.lengthdir_y(4,30),ID).setID(Iw+1));
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,-30),mY+(int)APPLET.lengthdir_y(4,-30),ID).setID(Iw+2));
                                Server.MYID+=2;
                             }
                         if (number > 5)
                             {
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,45),mY+(int)APPLET.lengthdir_y(4,45),ID).setID(Iw+3));
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,-45),mY+(int)APPLET.lengthdir_y(4,-45),ID).setID(Iw+4));
                                Server.MYID+=2;
                             }
                         if (number > 7)
                             {
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,60),mY+(int)APPLET.lengthdir_y(4,60),ID).setID(Iw+5));
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,-60),mY+(int)APPLET.lengthdir_y(4,-60),ID).setID(Iw+6));
                                Server.MYID+=2;
                             }
                         if (number > 12)
                             {
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,15),mY+(int)APPLET.lengthdir_y(4,15),ID).setID(Iw+7));
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,-15),mY+(int)APPLET.lengthdir_y(4,-15),ID).setID(Iw+8));
                                Server.MYID+=2;
                             }
                         if (number > 16)
                             {
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,35),mY+(int)APPLET.lengthdir_y(4,35),ID).setID(Iw+9));
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,-35),mY+(int)APPLET.lengthdir_y(4,-35),ID).setID(Iw+10));
                                Server.MYID+=2;
                             }
                         if (number > 20)
                             {
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,45),mY+(int)APPLET.lengthdir_y(4,45),ID).setID(Iw+11));
                                handle.earth.entityList.add(new SandEntity(Xx,Yy,mX+(int)APPLET.lengthdir_x(4,-45),mY+(int)APPLET.lengthdir_y(4,-45),ID).setID(Iw+12));
                                Server.MYID+=2;
                             }
                         handle.sendMessage(Server.EARTHBENDING,ByteBuffer.allocate(32).putInt(subID).putInt(Xx).putInt(Yy).putInt(mX).putInt(mY).putInt(ID).putInt(Iw).putInt(number));
                         Server.MYID+=2;
                     break;
                      case 5:
                         //handle.earth.entityList.add(new EffectEntity(Xx,Yy,mX,mY,ma));
                 ...
                 if (subID!=4)
                 {
                    handle.sendMessage(Server.EARTHBENDING,ByteBuffer.allocate(28).putInt(subID).putInt(Xx).putInt(Yy).putInt(mX).putInt(mY).putInt(ID).putInt(Iw));
                 }
            break;
            case Server.WATERBENDING:  
                 ...
                 handle.sendMessage(Server.WATERBENDING,ByteBuffer.allocate(28).putInt(subID).putInt(Xx).putInt(Yy).putInt(mX).putInt(mY).putInt(ID).putInt(Iw));
            break;
            case Server.LIGHTNING:  
                 ...
                 handle.sendMessage(Server.LIGHTNING,ByteBuffer.allocate(28).putInt(subID).putInt(Xx).putInt(Yy).putInt(mX).putInt(mY).putInt(ID).putInt(Iw));
            break;
            ...
            case Server.DARKNESS:  
                 ...
                 handle.sendMessage(Server.DARKNESS,ByteBuffer.allocate(28).putInt(subID).putInt(Xx).putInt(Yy).putInt(mX).putInt(mY).putInt(ID).putInt(Iw));
            break;
            case Server.DEATH:  
                 buf = Server.readByteBuffer(in);
                 subID = buf.getInt();
    ...
    public void writeNewPlayer(int id, String user)
    {
        try 
        {
            ...
            //out.write(id);
            //out.flush();
            out.addMesssage(Server.putString(ByteBuffer.allocate(4+user.length()*4).putInt(id),user), Server.LOGIN);
        } 
        catch (Exception ex) {
            //ex.printStackTrace();
